...
@timeit
@memprofit
def read_positions(u, atoms, b=0, e=10000000, skip_rate=1, dtype=np.float32):
    logger.info("Reading positions...")
    arr = np.array(
        [atoms.positions.flatten() for ts in u.trajectory[::skip_rate] if in_range(ts, b, e)], dtype=dtype)
    arr = np.ascontiguousarray(arr.T)  # Transpose for memory efficiency (shape: (n_coords, n_frames))
    logger.info("Done!")
    return arr


def read_velocities(u, atoms, b=0, e=10000000, skip_rate=1, dtype=np.float32):
    logger.info("Reading velocities...")
    arr = np.array(
        [atoms.velocities.flatten() for ts in u.trajectory[::skip_rate] if in_range(ts, b, e)], dtype=dtype)
    arr = np.ascontiguousarray(arr.T)  # Transpose for memory efficiency (shape: (n_coords, n_frames))
    logger.info("Done!")
    return arr
