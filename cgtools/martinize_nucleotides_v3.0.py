...
# Residue classes:
nucleic = spl("DA DC DG DT A C G U \
...
class elnedyn30nucleic():
    ...
    def __init__(self):
        ...
        # Charged types:
        charges = {"TDU":0.5,   "TA1":0.4, "TA2":-0.4, "TA3":0.4, "TA4":-1.1, "TA5":1.1, "TA6":-0.4, 
                                "TY1":0.0, "TY2":-0.5, "TY3":-0.7, "TY4":0.7, "TY5":0.5,
                                "TG1":0.4, "TG2":-0.5, "TG3":0.6, "TG4":-0.5, "TG5":-0.8, "TG6":-0.5, "TG7":0.5, "TG8":0.8,
                                "TU1":0.0, "TU2":-0.6, "TU3":-0.7, "TU4":-0.8, "TU5":0.6, "TU6":0.7, "TU7":0.8,}  
        ...
        self.rna_bb = {
            ...
            'bond'  : [(1,  0.351, 20000),          
                       ...
                       (1,  0.402, 12000)],          
            'angle' : [(10,  115.0, 30),       
                       (10,  120.0, 110)],    # TODO UPDATE ACCORDING TO THE DISTRIBUTION       
            'dih'   : [(3,   3,  -3, 6, 2, -6, -2),  # (3,   6,  -4, 11, 4, -13.0, -4)  # (3,   6,  -5, 9, 5, -9.0, -6)
                       (1,     15.0,   7, 1),],  # (1,     15.0,   5, 1)
            'excl'  : [(), (), ()],
            'pair'  : [],
        }
        # RNA BACKBONE CONNECTIVITY
    ...
    def split(self):
        ...
        for i in range(len(self.sequence)-1):
            if residueTypes.get(self.sequence[i],"Unknown") != residueTypes.get(self.sequence[i+1],"Unknown"):
                ...
                chainStart = i+1
        if chains:
            logging.debug('Splitting chain %s in %s chains'%(self._id,len(chains)+1))
...
class Vsite(Bonded):
    ...
    def __str__(self):
        if not self.atoms:
            return ""
        # s = ["%5d" % i for i in self.atoms]
        s = [formatString(self.atoms[0]), formatString(self.atoms[1]), formatString(self.atoms[2]), formatString(self.atoms[3]), 
                formatString(self.parameters[0]), formatString(self.parameters[1]), formatString(self.parameters[2])]
        if self.comments:
            s.append(';')
            if type(self.comments) == str:
                s.append(self.comments)
        ...
        if options["-x"].value:
            ...
            for i in order:
                ...
                if coarseGrained:
                    ...
                    logging.warning("No mapping for coarse graining chain %s (%s); chain is skipped."%(ci._id,ci.type()))
