...
import itpio
import cgmap
from geometry import get_angle, get_bonds, get_angles, get_dihedrals
from plot_geometry import plot_geometry, make_fig_one
...
def shizz_to_get_done(all_bonds, all_angles, all_dihedrals, atom_dict, resname, next_dict, next_resname, mapping, topology):
    if mapping:
        ...
        for chain in model:
            ...
            for resi, residue in enumerate(residues):
                ...
                if resname in resnames:
                    shizz_to_get_done(bonds, angles, dihedrals, atom_dict, resname, next_dict, next_resname, mapping, topology[resname])
    return bonds, angles, dihedrals
...
def update_topology(topology, bonds, angles, dihedrals, std_bonds, std_angles, std_dihedrals):
    ...
    for section, bonds, stds, fc, ftype in zip(sections, glist, stdlist, fcs, ftypes):
        for key, bond, std in zip(topology[section].keys(), bonds, stds):
            if topology[section][key][2] != 100000.0:
...
def bonded_parameters():
    ...
    molecule = "test"
    system = sys.argv[1]
    topology = read_topology(   a_itp=f'itp/nucleotides/RNA_A_{version}.itp', 
                                c_itp=f'itp/nucleotides/RNA_C_{version}.itp', 
                                g_itp=f'itp/nucleotides/RNA_G_{version}.itp', 
                                u_itp=f'itp/nucleotides/RNA_U_{version}.itp')
    ...
    cg_structure = make_structure_pdb(f"systems/{system}/test.pdb")
    ...
    for resname in resnames:
        ...
        all_aa_bonds.append(bonds)
        all_aa_angles.append(angles)
        all_aa_dihedrals(dihedrals)
        all_cg_bonds.append(cgbonds)
        all_cg_angles.append(cgangles)
        all_cg_dihedrals(cgdihedrals)
        ...
        # itpio.write_itp(f"itp/working/{name}.itp", topology[resname]) 
