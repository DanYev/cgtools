...
class mmSystem:
    ...
    @staticmethod
    def forcefield(
        force_field="amber14-all.xml", water_model="amber14/tip3p.xml", **kwargs
    ):
    ...
    def model(self, forcefield, modeller, barostat=None, thermostat=None, **kwargs):
        ...
        with open(self.syspdb, "w") as file:
            app.PDBFile.writeFile(
                modeller.topology, modeller.positions, file, keepIds=True
            )
...
class mdRun(mmSystem):
    ...
    def md(
        self, simulation, nsteps=100000, nout=1000, nlog=10000, nchk=10000, **kwargs
    ):
