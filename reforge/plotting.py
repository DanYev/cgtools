...
def make_heatmap(data, outfile="png/heatmap.png"):
    ...
    fig, ax = plt.subplots()
    img = ax.imshow(data, cmap='bwr', vmin=0.0, vmax=2.0, )
    xmax = data.shape[0] - 1
    ymax = data.shape[1] - 1
    ...
    ax.hlines(line_positions, xmin=0, xmax=ymax, colors='black', linestyles='dashed', linewidth=0.8, alpha=1.0)  
    fig.savefig(outfile)
    return fig, img
...
def make_plot(datas, labels, outfile="png/test.png"):
    ...
    ax.vlines(line_positions, ymin=ymin, ymax=ymax, colors='black', linestyles='dashed', linewidth=0.8, alpha=1.0)
...
def make_plot_t(datas, labels, outfile="png/test.png", dt=0.2):
    ...
    resls = ['D10', 'S15', 'E762', 'H840', 'H983', 'D986',] #  ['R1333', 'R1335']
    fig, ax = plt.subplots(figsize=(12, 5))
    colors = ['k', 'red']
    for data, label, color in zip(datas, labels, colors):
        data = np.array(data)
        for residue, resl in zip(residues, resls):
            x = np.arange(data.shape[0]) * dt  # Create x-axis values from 0 to len(data)-1
            ax.plot(x[:400], data[:400, residue], label=f'{label}_{resl}', lw=2, color=color)                 # Initial plot
    ymin, ymax = 0, 1.2
    ax.set_ylim(ymin, ymax)
    # xmin, xmax = 1, 1368
    # ax.set_xlim(xmin, xmax)
...
def make_plot_t_2d(datas, labels, outfile="png/test.png", dt=0.2, nframes=750):
    ...
    for data, label, ls in zip(datas, labels, lss):
        ...
        for i, color, resl in zip(range(6), colors, resls):
            x = np.arange(data.shape[0]) * dt  # Create x-axis values from 0 to len(data)-1
            ax.plot(x[:nframes], data[:nframes, 1333, i], label=f'{label}_{resl}', lw=2, color=color, linestyle=ls)                 # Initial plot
    ymin, ymax = 0, 3
    ax.set_ylim(ymin, ymax)
    # xmin, xmax = 1, 1368
    # ax.set_xlim(xmin, xmax)
...
def animate_1d(fig, ax, lines, datas, outfile="data/ani1d.mp4", dt=0.2):
    ...
    def update(frame):
        ...
        return tuple(lines)
    ani = animation.FuncAnimation(fig, update, frames=len(datas[0]), interval=50, blit=False)  
...
def mkfigure(datas, labels):
    ...
    figure = Graph(datas, labels, legend=True, loc='upper right', size=(6.2, 5), fontsize=13, 
                xlabel='Perturbed Residue', ylabel='Coupled Residue')
    return figure
